...
/// and are plotted by the percentage of each value to the
/// total amount in each group.
///
/// Provides options to customize the [color], [opacity], [borderWidth],
/// [borderColor], [borderDrawMode] of the stacked area 100 series segments.
///
...
class StackedArea100Series<T, D> extends StackedSeriesBase<T, D> {
  ...
  @override
  StackedArea100SeriesRenderer<T, D> createRenderer() {
    StackedArea100SeriesRenderer<T, D> stackedAreaSeriesRenderer;
    if (onCreateRenderer != null) {
      stackedAreaSeriesRenderer =
          onCreateRenderer!(this) as StackedArea100SeriesRenderer<T, D>;
      return stackedAreaSeriesRenderer;
    }
    return StackedArea100SeriesRenderer<T, D>();
  }

  @override
  StackedArea100SeriesRenderer<T, D> createRenderObject(BuildContext context) {
    final StackedArea100SeriesRenderer<T, D> renderer =
        super.createRenderObject(context) as StackedArea100SeriesRenderer<T, D>;
    renderer
...
class StackedArea100SeriesRenderer<T, D> extends StackedSeriesRenderer<T, D>
    with ContinuousSeriesMixin<T, D>, Stacking100SeriesMixin<T, D> {
  ...
  @override
  StackedArea100Segment<T, D> createSegment() => StackedArea100Segment<T, D>();

  @override
  ShapeMarkerType effectiveLegendIconType() =>
      ShapeMarkerType.stackedArea100Series;
  ...
  List<ChartSegment> contains(Offset position) {
    ...
    for (final ChartSegment segment in segments) {
      if (segment is StackedArea100Segment<T, D>) {
...
class StackedArea100Segment<T, D> extends ChartSegment {
  late StackedArea100SeriesRenderer<T, D> series;
