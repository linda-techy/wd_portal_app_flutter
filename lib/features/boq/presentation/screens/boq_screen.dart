...
import 'package:admin/services/ai_boq_suggester_service.dart';
            ...
            content: SizedBox(
              ...
              child: SingleChildScrollView(
                child: Column(
                  ...
                  children: [
                    _AiSuggestPanel(
                      onSuggestionApplied: (suggestion) => setDialogState(() {
                        descController.text = suggestion.description;
                        hsnSacController.text = suggestion.hsnSacCode;
                        unitController.text = suggestion.unit;
                        // Honour the AI's BASE/ADDON classification — user can
                        // override via the existing dropdown if needed.
                        selectedItemKind = suggestion.itemKind;
                      }),
                    ),
                    const SizedBox(height: 12),
...
/// Inline AI assist panel for the BOQ create dialog. The site engineer or
/// estimator types a free-text description ("RCC slab for ground floor, 4 inch
/// thickness") and gets back a normalised draft — description, unit, HSN/SAC
/// code, item kind — that they can either apply with one tap or ignore.
///
/// The widget intentionally does NOT auto-apply: every suggestion shows a
/// summary card with the model's confidence and reasoning so the user can
/// audit it before accepting. Tax-invoice fields like HSN/SAC are high-stakes
/// to autofill silently.
class _AiSuggestPanel extends StatefulWidget {
  const _AiSuggestPanel({required this.onSuggestionApplied});

  final ValueChanged<BoqSuggestion> onSuggestionApplied;

  @override
  State<_AiSuggestPanel> createState() => _AiSuggestPanelState();
}

class _AiSuggestPanelState extends State<_AiSuggestPanel> {
  final TextEditingController _ctrl = TextEditingController();
  final AiBoqSuggesterService _ai = AiBoqSuggesterService(ApiService());
  BoqSuggestion? _suggestion;
  bool _loading = false;
  String? _error;
  ...
  Future<void> _suggest() async {
    ...
    try {
      final s = await _ai.suggest(text);
      if (!mounted) return;
      setState(() {
        _suggestion = s;
        _loading = false;
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        borderRadius: BorderRadius.circular(8),
        border: Border.all(color: const Color(0xFFD3C4FF)),
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
            children: [
              const Icon(Icons.auto_awesome, size: 18, color: Color(0xFF5B3FB9)),
              const SizedBox(width: 6),
              const Text('AI assist',
                  style: TextStyle(
                      fontWeight: FontWeight.w600,
                      fontSize: 13,
                      color: Color(0xFF5B3FB9))),
              const SizedBox(width: 8),
              Expanded(
                child: Text(
                  'Describe the work in your own words — pulls description, '
                  'unit, HSN/SAC code',
                  style: TextStyle(fontSize: 11, color: Colors.grey[700]),
                  overflow: TextOverflow.ellipsis,
                ),
            ...
            children: [
              Expanded(
                child: TextField(
                  ...
                  decoration: const InputDecoration(
                    isDense: true,
                    hintText: 'e.g. RCC slab for ground floor, 4 inch thickness',
              ...
              ElevatedButton.icon(
                ...
                label: Text(_loading ? 'Thinking…' : 'Suggest'),
                style: ElevatedButton.styleFrom(
                  backgroundColor: const Color(0xFF5B3FB9),
                  foregroundColor: Colors.white,
                  padding: const EdgeInsets.symmetric(
          ...
          if (_suggestion != null) ...[
            ...
            _SuggestionCard(
              ...
              onApply: () {
                ...
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(
                    content: Text('AI suggestion applied — review before saving'),
                    duration: Duration(seconds: 2),
                  ),
                );
              },
...
/// Review-and-apply card for an AI suggestion. Surfaces confidence, the
/// short reasoning, and the prompt version that produced it so the
/// suggestion is auditable rather than magic.
class _SuggestionCard extends StatelessWidget {
  const _SuggestionCard({required this.suggestion, required this.onApply});

  final BoqSuggestion suggestion;
  final VoidCallback onApply;

  Color get _confidenceColor {
    switch (suggestion.confidence.toLowerCase()) {
      case 'high':
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          Row(
            children: [
              ...
              Container(
                ...
                child: Text('${suggestion.promptVersion} · ${suggestion.providerName}',
                    style: TextStyle(
              ...
              TextButton.icon(
                ...
                style: TextButton.styleFrom(
                  foregroundColor: const Color(0xFF5B3FB9),
                  padding:
          ...
          const SizedBox(height: 6),
          _kv('Description', suggestion.description),
          _kv('Unit', suggestion.unit),
          _kv('HSN/SAC', suggestion.hsnSacCode),
          _kv('Item kind', suggestion.itemKind),
          const SizedBox(height: 6),
