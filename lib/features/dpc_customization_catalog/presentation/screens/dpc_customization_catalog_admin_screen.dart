...
/// Admin CRUD screen for the master DPC Customization Catalog.
///
/// Backed by `GET/POST/PATCH/DELETE /api/dpc-customization-catalog` —
/// listing, creation, edit (via dialog), soft-delete, and quick toggle of
/// `isActive`.
class DpcCustomizationCatalogAdminScreen extends StatefulWidget {
  const DpcCustomizationCatalogAdminScreen({super.key});

  @override
  State<DpcCustomizationCatalogAdminScreen> createState() =>
      _DpcCustomizationCatalogAdminScreenState();
}

class _DpcCustomizationCatalogAdminScreenState
    extends State<DpcCustomizationCatalogAdminScreen> {
  final DpcCustomizationCatalogService _service =
      DpcCustomizationCatalogService();

  // Indian-locale rupee formatter (no decimals — large catalog amounts read
  ...
  Future<void> _openCreateDialog() async {
    final created = await showDialog<DpcCustomizationCatalogItem>(
      context: context,
  ...
  Future<void> _openEditDialog(DpcCustomizationCatalogItem item) async {
    final saved = await showDialog<DpcCustomizationCatalogItem>(
      context: context,
      builder: (_) => _CatalogItemEditDialog(
        service: _service,
        existing: item,
  ...
  Future<void> _toggleActive(DpcCustomizationCatalogItem item) async {
    try {
  ...
  Future<void> _confirmDelete(DpcCustomizationCatalogItem item) async {
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: Text(
          ...
          'It will be hidden from the picker but historical DPC '
  ...
  Widget build(BuildContext context) {
    ...
    final canManage = permProvider.canManageDpcCustomizationCatalog;

    return Scaffold(
      backgroundColor: bgColor,
      appBar: AppBar(
        title: const Text('DPC Customization Catalog'),
        actions: [
          IconButton(
              ...
              columns: [
                ...
                const DataColumn(
                    label: Text('Unit',
                        style: TextStyle(fontWeight: FontWeight.bold))),
                DataColumn(
                  label: const Text('Default Amount',
                      style: TextStyle(fontWeight: FontWeight.bold)),
                  numeric: true,
                  onSort: (_, __) => _toggleSort('defaultAmount'),
                ),
                DataColumn(
  ...
  DataRow _buildRow(DpcCustomizationCatalogItem it, bool canManage) {
    return DataRow(
      cells: [
        DataCell(
          Container(
        ...
        DataCell(Text(_currency.format(it.defaultAmount))),
        DataCell(
          Container(
...
class _CatalogItemEditDialog extends StatefulWidget {
  final DpcCustomizationCatalogService service;
  final DpcCustomizationCatalogItem? existing;
...
class _CatalogItemEditDialogState extends State<_CatalogItemEditDialog> {
  ...
  void initState() {
    ...
    _amountCtrl = TextEditingController(
      text: e != null ? e.defaultAmount.toString() : '',
    );
  ...
  Future<void> _submit() async {
    ...
    try {
      ...
      final DpcCustomizationCatalogItem result;
      if (_isEdit) {
                ...
                Autocomplete<String>(
                  ...
                  fieldViewBuilder:
                    ...
                    return TextFormField(
                      ...
                      decoration: const InputDecoration(
                        labelText: 'Category',
                        hintText: 'e.g. Joinery, Pooja Room, Modular',
                        border: OutlineInputBorder(),
                        isDense: true,
                      ),
                    );
