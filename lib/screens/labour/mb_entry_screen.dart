...
class _MBEntryScreenState extends State<MBEntryScreen> {
  ...
  double get _totalAmount => (double.tryParse(_qtyController.text) ?? 0) * (double.tryParse(_rateController.text) ?? 0);
      ...
      body: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              _buildTextField(_descController, "Work Description", Icons.description, required: true),
              const SizedBox(height: 16),
              DropdownButtonFormField<Labour>(
                value: _selectedLabour,
                decoration: const InputDecoration(labelText: "Labour/Group (Optional)", prefixIcon: Icon(Icons.people)),
                items: labourList.map((l) => DropdownMenuItem(value: l, child: Text(l.name))).toList(),
              ...
              Row(
                children: [
                  Expanded(child: _buildNumField(_qtyController, "Quantity", required: true)),
                  const SizedBox(width: 8),
                  Expanded(
                    child: DropdownButtonFormField<String>(
                      value: _unit,
                      decoration: const InputDecoration(labelText: "Unit"),
                      items: ['SFT', 'CFT', 'SQM', 'CUM', 'RMT', 'NOS'].map((u) => DropdownMenuItem(value: u, child: Text(u))).toList(),
              ...
              Card(
                ...
                child: Padding(
                  ...
                  child: Row(
                    ...
                    children: [
                      const Text("Total Final Amount", style: TextStyle(fontWeight: FontWeight.bold)),
                      Text("₹${_totalAmount.toStringAsFixed(2)}", style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 18, color: AppTheme.deepSlate)),
                    ],
                  ),
                ),
              ),
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  child: const Text("Commit MB Record", style: TextStyle(color: Colors.white, fontSize: 16)),
                ),
              ),
            ],
          ),
  ...
  Widget _buildNumField(TextEditingController controller, String label, {bool required = false}) {
    return TextFormField(
      controller: controller,
  ...
  void _saveMB() async {
    if (_formKey.currentState!.validate()) {
      ...
      if (success) {
        if (mounted) {
          ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text("MB Entry saved!")));
