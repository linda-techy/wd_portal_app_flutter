...
class PaymentHistoryScreen extends StatelessWidget {
  ...
  Widget _buildPaymentCard(BuildContext context, dynamic payment) {
    // Dynamic type because payment can be different models
    final amount = payment.amount ?? 0.0;
    final status = payment.status ?? 'UNKNOWN';
    final date = payment.paymentDate ?? payment.createdAt;
    final reference = payment.referenceNumber ?? payment.transactionId ?? 'N/A';
  ...
  void _navigateToDetail(BuildContext context, dynamic payment) {
    ScaffoldMessenger.of(context).showSnackBar(
      const SnackBar(content: Text('View payment details')),
    );
  }
}
