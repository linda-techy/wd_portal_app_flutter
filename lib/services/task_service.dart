...
class TaskService {
  ...
  Future<TaskModel> getTaskById(int id) async {
    ...
    return _apiService.unwrap(response, (json) => TaskModel.fromJson(json as Map<String, dynamic>));
  }
  ...
  Future<List<TaskModel>> getTasksByLead(int leadId) async {
    final response = await _apiService.get('/tasks/by-lead/$leadId');
    return _apiService.unwrapList(response, (json) => TaskModel.fromJson(json));
  }

  /// Get assignment history for a task
  Future<List<TaskAssignmentHistoryModel>> getAssignmentHistory(int taskId) async {
    final response = await _apiService.get('/tasks/$taskId/assignment-history');
    return _apiService.unwrapList(response, (json) => TaskAssignmentHistoryModel.fromJson(json));
  }

  /// Create new task
  Future<TaskModel> createTask(CreateTaskRequest request) async {
    final response = await _apiService.post('/tasks', data: request.toJson());
    return _apiService.unwrap(response, (json) => TaskModel.fromJson(json as Map<String, dynamic>));
  }
  ...
  Future<TaskModel> updateTask(int id, UpdateTaskRequest request) async {
    final response = await _apiService.put('/tasks/$id', data: request.toJson());
    return _apiService.unwrap(response, (json) => TaskModel.fromJson(json as Map<String, dynamic>));
  }
  ...
  Future<TaskModel> assignTask(int taskId, int? userId, {String? notes}) async {
    final response = await _apiService.put('/tasks/$taskId/assign', data: {
      'userId': userId,
  ...
  Future<Map<String, dynamic>> getAlertStats({int days = 7}) async {
    final response = await _apiService.get('/tasks/alerts/stats', queryParams: {'days': days.toString()});
    return _apiService.unwrap(response, (json) => json as Map<String, dynamic>);
  }

  /// Get recent alerts
  Future<List<TaskAlertModel>> getRecentAlerts() async {
    final response = await _apiService.get('/tasks/alerts/recent');
    return _apiService.unwrapList(response, (json) => TaskAlertModel.fromJson(json));
  }

  /// Trigger manual alerts (Admin only)
  Future<void> triggerAlerts() async {
    final response = await _apiService.post('/tasks/alerts/trigger', data: {});
    _apiService.unwrap(response, (_) {});
  }
}
